// Node.js runner for performance hypothesis tests
// Runs the same A/B comparisons without a browser
...
function bench(name, fn, iterations = 100) {
  ...
  return { name, median, p5, p95, iterations };
}
...
function printComparison(baseline, optimized) {
  ...
  const marker = speedup > 1.1 ? '\x1b[32m ✓ FASTER\x1b[0m' :
                 speedup < 0.95 ? '\x1b[31m ✗ SLOWER\x1b[0m' :
                 '\x1b[33m ─ SAME\x1b[0m';
  console.log(
...
// ═══════════════════════════════════════════════════════════════
// H1: KD-Tree build — sort+slice vs quickselect+indices
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// H2: SpatialHash — Array-of-Arrays vs Flat Int32Array
// ═══════════════════════════════════════════════════════════════
  ...
  query(x, y, radius) {
    ...
    for (let row = minRow; row <= maxRow; row++) {
      for (let col = minCol; col <= maxCol; col++) {
        ...
        for (let i = 0; i < cnt; i++) buf[qc++] = this.data[base + i];
      }
    }
    this._queryCount = qc;
    return qc;
...
// ═══════════════════════════════════════════════════════════════
// H3: Greedy assignment — brute vs spatial
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// H4: Physics repulsion — sqrt vs no-sqrt
// ═══════════════════════════════════════════════════════════════
...
function physicsStep_current(positions, velocities, assigned, count, repRadius, repulsion, w, h) {
  ...
  for (let i = 0; i < count; i++) {
    const px = positions[i * 2], py = positions[i * 2 + 1];
    let vx = (assigned[i * 2] - px) * 0.32;
    let vy = (assigned[i * 2 + 1] - py) * 0.32;
    const neighbors = hash.query(px, py, repRadius);
    for (const ni of neighbors) {
      if (ni === i) continue;
      const ndx = px - positions[ni * 2], ndy = py - positions[ni * 2 + 1];
...
// ═══════════════════════════════════════════════════════════════
// H5: resolveOverlaps — array-of-arrays spiral vs flat Int8Array
// ═══════════════════════════════════════════════════════════════
...
function resolveOverlaps_current(positions, count, w, h) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const ci = ry * w + rx;
    if (grid[ci] === -1) {
      grid[ci] = i; positions[i * 2] = rx; positions[i * 2 + 1] = ry;
    } else {
      for (let s = 0; s < SPIRAL_OFFSETS.length; s++) {
        const nx = rx + SPIRAL_OFFSETS[s][0], ny = ry + SPIRAL_OFFSETS[s][1];
        if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
        const ni = ny * w + nx;
        if (grid[ni] === -1) {
          grid[ni] = i; positions[i * 2] = nx; positions[i * 2 + 1] = ny; break;
        }
      }
    }
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (grid[ci] === -1) {
      ...
      for (let s = 0; s < slen; s++) {
        const nx = rx + SPIRAL_DX[s], ny = ry + SPIRAL_DY[s];
...
// ═══════════════════════════════════════════════════════════════
// H6: swapOptimize — Array hash vs Flat hash
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// H7: Pipeline lightest-color — pixel scan vs palette scan
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// H8: Floyd-Steinberg — alloc vs pooled
// ═══════════════════════════════════════════════════════════════
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      for (let k = 0; k < offsets.length; k++) {
        const nx = x + offsets[k][0], ny_ = y + offsets[k][1];
        if (nx >= 0 && nx < w && ny_ >= 0 && ny_ < h) {
          const nj = (ny_ * w + nx) * 3;
...
let _pooledResult = null, _pooledResultSize = 0;
let _pooledErr = null, _pooledErrSize = 0;

function ditherFS_pooled(pixels, w, h, lut) {
  const offsets = FLOYD_STEINBERG.offsets;
  const errSize = w * h * 3;
  if (!_pooledErr || _pooledErrSize < errSize) {
    _pooledErr = new Float32Array(errSize); _pooledErrSize = errSize;
  } else { _pooledErr.fill(0); }
  ...
  if (!_pooledResult || _pooledResultSize < resSize) {
    _pooledResult = new Uint8ClampedArray(resSize); _pooledResultSize = resSize;
  }
...
console.log('Dither Studio — Performance Hypothesis Testing');
console.log('═'.repeat(80));
console.log('  ' + pad('Test', 28) + padL('Current', 12) + padL('Optimized', 12) + padL('Speedup', 8));
console.log('─'.repeat(80));

// H1
console.log('\n\x1b[33m▸ H1: KD-Tree Build (sort+slice vs quickselect+indices)\x1b[0m');
...
// H2
console.log('\n\x1b[33m▸ H2: Spatial Hash (Array-of-Arrays vs Flat Int32Array)\x1b[0m');
...
// H3
console.log('\n\x1b[33m▸ H3: Greedy Assignment (brute O(n²) vs spatial-hash O(n·k))\x1b[0m');
for (const count of [1000, 3000, 5000]) {
  const w = 100, h = 100;
  const positions = makeRandomPositions(count, w, h);
  const targets = makeRandomPositions(count, w, h);
  const iters = count > 3000 ? 10 : 20;
  const baseline = bench(`Greedy ${count}`, () => assignGreedy_current(positions, targets, count, count), iters);
  const optimized = bench(`Greedy ${count}`, () => assignGreedy_spatial(positions, targets, count, count, w, h), iters);
  printComparison(baseline, optimized);
}

// H4
console.log('\n\x1b[33m▸ H4: Physics Repulsion (sqrt vs no-sqrt approximation)\x1b[0m');
for (const count of [2000, 5000]) {
  const w = 100, h = 100;
  const positions = makeRandomPositions(count, w, h);
  ...
  const baseline = bench(`Physics ${count}`, () => {
    physicsStep_current(new Float32Array(positions), velocities, assigned, count, 2, 1.0, w, h);
  }, 30);
  const optimized = bench(`Physics ${count}`, () => {
    physicsStep_nosqrt(new Float32Array(positions), velocities, assigned, count, 2, 1.0, w, h);
  }, 30);
  printComparison(baseline, optimized);
}

// H5
console.log('\n\x1b[33m▸ H5: resolveOverlaps (array spiral vs flat Int8Array spiral)\x1b[0m');
for (const [count, w, h] of [[2000, 60, 60], [5000, 100, 100]]) {
  const positions = makeRandomPositions(count, w, h);
  const baseline = bench(`Overlaps ${count}`, () => resolveOverlaps_current(new Float32Array(positions), count, w, h), 50);
  const optimized = bench(`Overlaps ${count}`, () => resolveOverlaps_flat(new Float32Array(positions), count, w, h), 50);
  printComparison(baseline, optimized);
}

// H6
console.log('\n\x1b[33m▸ H6: swapOptimize (Array hash vs Flat hash)\x1b[0m');
for (const count of [2000, 5000]) {
  const w = 100, h = 100;
  const positions = makeRandomPositions(count, w, h);
  const assigned = makeRandomPositions(count, w, h);
  ...
  const baseline = bench(`Swap ${count}`, () => swapOptimize_current(positions, new Float32Array(assigned), new Int32Array(mapping), count, w, h), 30);
  const optimized = bench(`Swap ${count}`, () => swapOptimize_flat(positions, new Float32Array(assigned), new Int32Array(mapping), count, w, h), 30);
  printComparison(baseline, optimized);
}

// H7
console.log('\n\x1b[33m▸ H7: Pipeline BG Scan (per-frame pixel scan vs palette scan)\x1b[0m');
for (const [w, h] of [[91, 68], [160, 144]]) {
  ...
  const baseline = bench(`BG scan ${w}×${h}`, () => pipelineBgScan_current(dithered, w, h), 200);
  const optimized = bench(`BG scan ${w}×${h}`, () => pipelineBgScan_fromPalette(palette), 200);
  printComparison(baseline, optimized);
}

// H8
console.log('\n\x1b[33m▸ H8: Floyd-Steinberg (alloc per frame vs pooled buffers)\x1b[0m');
for (const [w, h] of [[91, 68], [160, 144]]) {
  const pixels = makeGradientImage(w, h);
  const lut = buildColorLUT(makePalette(4));
  const baseline = bench(`F-S alloc ${w}×${h}`, () => ditherFS_alloc(new Uint8ClampedArray(pixels), w, h, lut), 100);
  const optimized = bench(`F-S pool ${w}×${h}`, () => ditherFS_pooled(new Uint8ClampedArray(pixels), w, h, lut), 100);
  printComparison(baseline, optimized);
}

// H9
console.log('\n\x1b[33m▸ H9: kdNearest (object alloc per recursion vs mutable array)\x1b[0m');
for (const count of [1000, 3000, 5000]) {
  const targets = makeRandomPositions(count, 100, 100);
  const ids = Array.from({ length: count }, (_, i) => i);
  ...
  const baseline = bench(`kdNearest ${count}t`, () => {
    for (let q = 0; q < 100; q++) kdNearest_current(tree, [queries[q * 2], queries[q * 2 + 1]], -1, Infinity, new Set());
  }, 50);
  const optimized = bench(`kdNearest ${count}t`, () => {
    for (let q = 0; q < 100; q++) kdNearest_mutable(tree, [queries[q * 2], queries[q * 2 + 1]], -1, Infinity, new Set());
  }, 50);
  printComparison(baseline, optimized);
}

console.log('\n' + '═'.repeat(80));
console.log('Done. Apply optimizations marked ✓ FASTER to production code.');
